...
| v5.9 | 新增节点级 `controller` 字段，用于覆盖当前节点的目标控制器 |
...
- `controller`: *string* **💡 v5.9**  
    当前节点执行时使用的控制器名称。可选，默认空。  
    当设置后，该节点会优先使用指定的 controller；否则回退到任务默认 controller，再回退到接口默认 controller。  
    该覆盖仅作用于当前节点，不会向后续节点隐式继承。
...
- `timeout`: *int*  
    next 列表循环识别的超时时间，毫秒。可选，默认 20 * 1000（20 秒）。  
