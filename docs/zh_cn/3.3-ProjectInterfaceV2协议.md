...
| 2026-3-21 | v2.5.0 | 新增 `default_controller` 顶层/任务级字段，用于指定默认控制器 |
...
- default_controller `string` **💡 v2.5.0**  
  可选。项目级默认控制器名称，值应与 `controller[].name` 对应。  
  当任务未指定 `task.default_controller`、节点未指定 pipeline `controller` 时，将使用该控制器作为默认目标。
...
`task[].default_controller` **💡 v2.5.0** 用于指定该任务执行时默认使用的控制器名称，值应与 `controller[].name` 对应。

控制器生效优先级如下：

1. pipeline 节点 `controller`
2. `task.default_controller`
3. `interface.default_controller`

其中 `task.controller` 仍然只表示该任务对控制器的适用性过滤，不承担默认选择语义。
