...
| v5.9 | Added node-level `controller` field for overriding the target controller of the current node |
...
- `controller`: *string* **💡 v5.9**  
    The controller name used when executing the current node. Optional, empty by default.  
    When set, this node uses the specified controller first; otherwise it falls back to the task default controller, then the interface default controller.  
    This override only affects the current node and is not inherited by subsequent nodes.
...
- `timeout`: *int*  
    Timeout for loop recognition in the next list, in milliseconds. Optional, default is 20 * 1000 milliseconds (20 seconds).  
...
- `anchor`: *string* | *list<string, >* | *object* **💡 v5.1**  
    ...
    When the node executes successfully, the anchor name will be set to the corresponding node. Multiple nodes can set the same anchor name, with later executions overwriting earlier ones.  
